import {
  Component, AfterViewInit, ChangeDetectionStrategy, ChangeDetectorRef, EventEmitter, OnInit, SimpleChanges,
...
export class EmailTemplateComponent implements AfterViewInit,OnInit,OnChanges,OnDestroy {
  ...
  Save(){
      let template = jQuery("#destination").html();
  ...
  onImgBoxSizeChange(imgfrom:string,event:any){
    ...
    if(self.imgFrom=='left'){
      ...
      self.onImgSizeChange('null','actualwithtext');
  ...
  TestEmails(){
    ...
    let template = jQuery("#email").html();
  ...
  ColorPickerDisplay(event: any){
    ...
    setTimeout(()=>{
      ...
      jQuery(event.target).parent().children('color-picker').find('.color-picker').css('position', 'absolute');
      jQuery(event.target).parent().children('color-picker').find('.color-picker').css('top', '10px');
      jQuery(event.target).parent().children('color-picker').find('.color-picker').css('left', '-230px');
      jQuery(event.target).parent().children('color-picker').find('.color-picker').css('z-index', '9999');
    },0);
  ...
  ngAfterViewInit(){
    ...
    jQuery("[data-module]").addClass("indestination");
    ...
    jQuery("[data-module='discount_item']").on("click", function(event:any){
